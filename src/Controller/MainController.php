...
class MainController extends AbstractController
{
    ...
    public function index(VideoGameRepository $videoGameRepository, ConsoleRepository $consoleRepository): Response
    {
        ...
        if(!empty($consoleList)) {$consoleWithMostGames = array_search(max(array_count_values($consoleList)), array_count_values($consoleList));}
        else {$consoleWithMostGames = "";}

        $consoleListOpti = (array_count_values($consoleList));
        
        return $this->render('main/home_page.html.twig', [
            'controller_name' => 'MainController',
            'videoGames' => $videoGameRepository->findTwelveRandomlyByUser($currentUser),
